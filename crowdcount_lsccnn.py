...
class CrowdCounter(object):
    def __init__(
        ...
        with torch.no_grad():
            self.model.forward(
                torch.from_numpy(np.zeros((1, 3, 224, 224), dtype=np.float32)).cuda()
            )
        ...
        self.box_sizes, self.box_sizes_flat = compute_boxes_and_sizes(
    ...
    def predict(self, img, nms_thresh=None):
        ...
        pred_dot_map, pred_box_map = get_box_and_dot_maps(
            upsampled_out, nms_thresh, self.box_sizes, omit_scales=self.omit_scales
        )
    ...
    def visualise_count(self, frame, nms_thresh=None, omit_scales=[], ignore_polys=[]):
        ...
        boxed_img, recount = get_boxed_img(
            small_img,
            pred_box_map,
            pred_box_map,
            pred_dot_map,
            ...
            thickness=2,
            multi_colours=True,
            omit_scales=self.omit_scales,
            ignore_polys=self.ignore_polys,
        )
...
if __name__ == "__main__":
    ...
    b_wts = (
        "/home/levan/Workspace/lsc_cnn_docker/lsc-cnn/models/part_b_scale_4_epoch_24_weights.pth"
    )
    ...
    cv2.imwrite(
        os.path.join("outputs", os.path.basename(img_path).split(".")[0] + "_crowdcount.png"),
        show_img,
    )
