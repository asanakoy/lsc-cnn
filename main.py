...
SCALE_BINS_ON_BOX_SIZE_BINS = [NUM_BOXES_PER_SCALE * (s + 1) for s in range(len(GAMMA))]
...
parser.add_argument(
    "--patches", default=100, type=int, metavar="N", help="number of patches per image"
)
parser.add_argument("--dataset", default="parta", type=str, help="dataset to train on")
parser.add_argument(
    "--lr", "--learning-rate", default=1e-3, type=float, metavar="LR", help="initial learning rate"
)
parser.add_argument("--momentum", default=0.9, type=float, metavar="M", help="momentum")
parser.add_argument(
    "--threshold", default=-1.0, type=float, metavar="M", help="fixed threshold to do NMS"
)
parser.add_argument(
    "--weight-decay",
    "--wd",
...
parser.add_argument("--mle", action="store_true", help="calculate mle")
parser.add_argument("--lsccnn", action="store_true", help="use the vgg_modified network")
parser.add_argument(
    "--trained-model",
    default="",
    type=str,
...
class networkFunctions:
    ...
    def get_box_gt(self, Yss):
        ...
        for b in range(0, gt_ref_map.shape[0]):
            ...
            if num_heads > 1:
                distances = (x_idx - x_idx[np.newaxis, :].T) ** 2 + (
                    y_idx - y_idx[np.newaxis, :].T
                ) ** 2
            ...
            for i, (yss, w_map) in enumerate(zip(Yss, Yss_out)):
                ...
                if scale_sel_inds.shape[0] > 0:
                    ...
                    Yss_out[i][b, scale_box_inds, heads_y, heads_x] = BOX_SIZE_BINS_NPY[
                        sel_box_inds
                    ]
    ...
    def gwta_loss(self, pred, gt, criterion, grid_factor=2):
        ...
        pred_re = pred.unfold(2, int(patch_size.item()), stride).unfold(
            3, int(patch_size.item()), stride
        )
        gt_re = gt.unfold(1, int(patch_size.item()), stride).unfold(
            2, int(patch_size.item()), stride
        )

        pred_re = torch.reshape(
            pred, (pred.size(0), pred.size(1), num_grids, patch_size, patch_size)
        )  # Bx4xnum_gridxH'XW'
        gt_re = torch.reshape(
            gt, (gt.size(0), num_grids, patch_size, patch_size)
        )  # Bxnum_gridxH'XW'
    ...
    def create_network_functions(self, network):
        ...
        def train_function(Xs, Ys, hist_boxes, hist_boxes_gt, loss_weights, network):
            ...
            Yss_out = Yss_out[
                ::-1
            ]  # Reverse pred for uniformity of having lowest scale in the beginning
    ...
    if "scale_4" in net.name:
        ...
        for layer in cfg.keys():
            ...
            for name, module in net.named_children():
                if (
                    layer == name
                    and (not layer.startswith("conv_middle_"))
                    and (
                        not layer.startswith("conv_lowest_")
                        and (not layer.startswith("conv_scale1_"))
                    )
                ):
                    lyr = module
                    ...
                    layer.startswith("conv_middle_") or layer.startswith("conv_lowest_")
                ) and name == layer:
                    lyr = module
...
def get_offset_error(x_pred, y_pred, x_true, y_true, output_downscale, max_dist=16):
    ...
    off_err, num_hits, fn = offset_sum(
        sorted_idx, d, n, m, max_dist, hit_thresholds, len(hit_thresholds)
    )
    off_err /= n
...
def get_boxed_img(
    image, h_map, w_map, gt_pred_map, prediction_downscale, thickness=1, multi_colours=False
):
    """
    ...
    for y, x in zip(Y, X):
        ...
        cv2.rectangle(
            boxed_img,
            (
                max(int(prediction_downscale * x - w / 2), 0),
                max(int(prediction_downscale * y - h / 2), 0),
            ),
            (
                min(int(prediction_downscale * x + w - w / 2), W),
                min(int(prediction_downscale * y + h - h / 2), H),
            ),
            selected_colour,
            t,
        )
...
def test_lsccnn(test_funcs, dataset, set_name, network, print_output=False, thresh=0.2):
    ...
    if isinstance(print_output, str):
        ...
        print_path = os.path.join(RESULTS_DIR, "dump")
    else:
        print_path = None
    ...
    for e_idx, e_iter in tqdm(enumerate(e), desc="Test"):
        image_split = e_iter[1].split("/")
        image_name = image_split[len(image_split) - 1]
        image = cv2.imread(e_iter[1])
        maps = [(image, {}), (e_iter[2], {"cmap": "jet", "vmin": 0.0, "vmax": 1.0})]

        pred_dot_map, size = get_box_and_dot_maps(
            e_iter[0][0:4], thresh=thresh
        )  # prediction_downscale

        # -- Plotting boxes
        predicted_count = len(np.where(pred_dot_map > 0)[-1])
        # boxed_image_pred = get_boxed_img(image, size, size,
        #                                  pred_dot_map,
        ...
        gt_count = e_iter[4]
        print(
            f"- Image {os.path.basename(image_path)}: Predicted = {predicted_count} [GT {gt_count}]"
        )
...
def calculate_metrics(pred, true, metrics_test):
    ...
    if args.mle:
        if len(head_x_pred) == 0:
            ...
            off, _, _ = get_offset_error(
                head_x_pred, head_y_pred, head_x_true, head_y_true, output_downscale
            )
...
def find_class_threshold(f, iters, test_funcs, network, splits=10, beg=0.0, end=0.3):
    ...
    for li_idx in range(iters):
        ...
        for class_threshold in threshold:
            avg_error = test_lsccnn(
                test_funcs, dataset, "test_valid", network, True, thresh=class_threshold
            )
            avg_errors.append(avg_error[0]["new_mae"])
            log(
                f, "class threshold: %f, avg_error: %f" % (class_threshold, avg_error[0]["new_mae"])
            )
...
def train_networks(network, dataset, network_functions, log_path):
    ...
    if start_epoch > 0:
        losses_path = os.path.join(snapshot_path, "losses.pkl")
        if os.path.exists(losses_path):
            with open(losses_path, "rb") as lossfile:
        ...
        network, _ = load_net(
            network, network_functions, 0, snapshot_path, get_filename(network.name, start_epoch)
        )
    ...
    for e_i, epoch in enumerate(range(start_epoch, num_epochs)):
        ...
        for b_i in range(num_batches_per_epoch):
            ...
            losses, hist_boxes, hist_boxes_gt = train_funcs[0](
                Xs, Ys, hist_boxes, hist_boxes_gt, loss_weights, network
            )
        ...
        save_checkpoint(
            ...
            snapshot_path,
            get_filename(network.name, epoch + 1),
        )
    ...
    try:
        ...
        log(
            f,
            "Done Training.\n Minimum loss %s at epoch %s"
            % (valid_losses["new_mae"][min_epoch], min_epoch),
        )
    except:
        pass
    log(f, "\nTesting ...")
    _, txt = test_lsccnn(
        test_funcs, dataset, "test", network, join(RESULTS_DIR, "dump_test"), thresh=threshold
    )
...
def train():
    ...
    print("test,train: ", len(dataset.dataset_files["test"]), len(dataset.dataset_files["train"]))

    dataset.test_batch_size = 8
    global network
    NET_SCALE_FACTOR = 4
    network = LSCCNN(name="scale_4", output_downscale=NET_SCALE_FACTOR)
...
if __name__ == "__main__":
    ...
    if args.dataset == "parta":
        dataset_paths = {
            "test": [
                f"{DATASETS_ROOT}/ShanghaiTech/part_A/test_data/images",
                f"{DATASETS_ROOT}/ShanghaiTech/part_A/test_data/ground-truth",
            ],
            "train": [
                "../dataset/ShanghaiTech/part_A/train_data/images",
                f"{DATASETS_ROOT}/ShanghaiTech/part_A/train_data/ground-truth",
            ],
        }
        validation_set = 30
        ...
        dataset_paths = {
            "test": [
                f"{DATASETS_ROOT}/ShanghaiTech/part_B/test_data/images",
                f"{DATASETS_ROOT}/ShanghaiTech/part_B/test_data/ground-truth",
            ],
            "train": [
                f"{DATASETS_ROOT}/ShanghaiTech/part_B/train_data/images",
                f"{DATASETS_ROOT}/ShanghaiTech/part_B/train_data/ground-truth",
            ],
        }
        validation_set = 80
        output_downscale = 2
        ...
        dataset_paths = {
            "test": [
                f"{DATASETS_ROOT}/UCF-QNRF_ECCV18/Test/images",
                f"{DATASETS_ROOT}/UCF-QNRF_ECCV18/Test/ground_truth",
            ],
            "train": [
                f"{DATASETS_ROOT}/UCF-QNRF_ECCV18/Train/images",
                f"{DATASETS_ROOT}/UCF-QNRF_ECCV18/Train/ground_truth",
            ],
        }
        validation_set = 240
        output_downscale = 2
    ...
    model_save_dir = os.path.join("./models/", args.dataset)
    RESULTS_DIR = os.path.join("./outputs/", args.dataset)

    batch_size = args.batch_size
    crop_size = 224
    dataset = DataReader(path, name=args.dataset)
