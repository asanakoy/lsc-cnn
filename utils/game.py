...
def game_metric(level, gt, pred):
    ...
    gt_reshape_torch = gt_reshape_torch.unfold(0, int(h / float(num_cuts)), stride).unfold(
        1, int(h / float(num_cuts)), stride
    )
    pred_reshape_torch = pred_reshape_torch.unfold(0, int(h / float(num_cuts)), stride).unfold(
        1, int(h / float(num_cuts)), stride
    )
